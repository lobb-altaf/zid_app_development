...
class OauthRedirectView(View):
    def get(self, request, *args, **kwargs):
        return redirect(f"{BASE_URL}/oauth/authorize?client_id={CLIENT_ID}&response_type=code")
    ...
    def get(self, request, *args, **kwargs):
        ...
        if response.status_code == 200:
            ...
            token_data = TokenData(
                ...
                authorization_code=data['authorization'],
...
@csrf_exempt
def create_webhook(request):
    token_data = TokenData.objects.latest('created_at')
    if request.method == 'POST':
        api_url = "https://api.zid.sa/v1/managers/webhooks"
        ...
        webhook_data = { 
            # "event": "order.status.update",
            "event": "order.create",
            "target_url": "https://9087-2a06-5904-1e03-e100-58ff-e89c-186b-f8ed.ngrok-free.app/webhook/ ",
            "original_id": 3500,        
            "subscriber": "My Online Store App",
            "conditions": {
                "status": "new",
...
def create_an_invoice(invoice_data , seller_data , customer_data ,delivery_data, payment_data , products , discount_on_totals , charge_on_totals):
    ...
    # try:
        ...
        for i in each_kind:
            ...
            for product in the_products:
                ...
                product_total_without_tax = number_with_n_digits_after_the_point(round(the_price * the_quantity , 10))

                if product.get("charge_on_total"):
                    if product.get("charge_on_total").get("charge_value") > 0:
                        if product.get("charge_on_total").get("charge_type") == "percentage":
                            charge_on_total = number_with_n_digits_after_the_point(round(product_total_without_tax * (product.get("charge_on_total").get("charge_value") / 100) , 10))
                        else:
                            charge_on_total = number_with_n_digits_after_the_point(round(product.get("charge_on_total").get("charge_value") , 10))
                    else:
                        charge_on_total = 0.0
                else:
                    charge_on_total = 0.0
                ...
                if product.get("discount_on_total"):
                    if product.get("discount_on_total").get("discount_value") > 0:
                        if product.get("discount_on_total").get("discount_type") == "percentage":
                            discount_on_total = number_with_n_digits_after_the_point(round(product_total_without_tax * (product.get("discount_on_total").get("discount_value") / 100) , 10))
                        else:
                            discount_on_total = number_with_n_digits_after_the_point(round(product.get("discount_on_total").get("discount_value") , 10))
                    else:
                        discount_on_total = 0.0
                else:
                    discount_on_total = 0.0

                product_total_without_tax_with_discount_and_charge = number_with_n_digits_after_the_point(round((product_total_without_tax + charge_on_total) - discount_on_total , 10))

                the_total_of_products += product_total_without_tax_with_discount_and_charge

            each_kind_total.update({the_kind_of_products : number_with_n_digits_after_the_point(round(the_total_of_products , 10))})
        ...
        for i in each_tax:
            ...
            each_tax_total.update({the_tax_of_products : number_with_n_digits_after_the_point(round(the_total_of_products , 10))})
        ...
        def calculate_the_total_after_discount():
            ...
            return number_with_n_digits_after_the_point(round(total_of_the_invoice_after_discount , 10))
        ...
        def calculate_the_total_after_charge():
            ...
            return number_with_n_digits_after_the_point(round(total_of_the_invoice_after_charge , 10))
        ...
        def calculate_the_tax():
            ...
            for i in after_discount_each_tax_total:
                ...
                the_total_without_discount_and_without_charge_and_without_tax = each_tax_total.get(i)
                the_discounted_amount = number_with_n_digits_after_the_point(round(each_tax_total.get(i) - after_discount_each_tax_total.get(i) , 10))
                the_charged_amount = number_with_n_digits_after_the_point(round(after_charge_each_tax_total.get(i) - each_tax_total.get(i) , 10))

                the_total_with_discount_and_with_charge_and_without_tax = the_total_without_discount_and_without_charge_and_without_tax + the_charged_amount - the_discounted_amount
                the_total_tax = number_with_n_digits_after_the_point(round(the_total_with_discount_and_with_charge_and_without_tax * (i / 100) , 10))

                the_total_of_the_tax += the_total_tax
            
            the_total_of_the_tax = number_with_n_digits_after_the_point(round(the_total_of_the_tax , 10))
        ...
        if discount_on_totals.get("discount_type") == "percentage":
            the_percentage_of_the_discount_on_totals = discount_on_totals.get("discount_value")
        else:
            the_percentage_of_the_discount_on_totals = number_with_n_digits_after_the_point(round(((discount_on_totals.get("discount_value")/calculate_the_total_without_tax()) * 100),10))
        ...
        if charge_on_totals.get("charge_type") == "percentage":
            ...
            the_percentage_of_the_charge_on_totals = number_with_n_digits_after_the_point(round(((charge_on_totals.get("charge_value")/calculate_the_total_without_tax()) * 100),10))

        # claculate the the discount

        for i in each_kind_total:
            the_amount = each_kind_total.get(i)
            the_percentage_of_the_discount_on_totals_under_1 = the_percentage_of_the_discount_on_totals / 100
            the_final_amount = the_amount - (the_amount * the_percentage_of_the_discount_on_totals_under_1) # here is the problem
            after_discount_each_kind_total.update({i : number_with_n_digits_after_the_point(round(the_final_amount , 10))})


        for i in each_tax_total:
            the_amount = each_tax_total.get(i)
            the_percentage_of_the_discount_on_totals_under_1 = the_percentage_of_the_discount_on_totals / 100
            the_final_amount = the_amount - (the_amount * the_percentage_of_the_discount_on_totals_under_1)
            after_discount_each_tax_total.update({i : number_with_n_digits_after_the_point(round(the_final_amount , 10))})
        ...
        if discount_on_totals.get("discount_type") == "amount":
            ...
            the_discounted_amount = round(calculate_the_total_without_tax() - calculate_the_total_after_discount() , 10)
                ...
                if the_discount_amount > the_discounted_amount:
                    ...
                    for i in after_discount_each_tax_total:
                        if after_discount_each_tax_total.get(i) > the_rest:
                            ...
                            after_discount_each_tax_total.update({i : number_with_n_digits_after_the_point(round(the_new_amount , 10))})
                    ...
                    if not discounting_status:
                        for i in after_discount_each_kind_total:
                            if after_discount_each_kind_total.get(i) > the_rest:
                                the_new_amount = after_discount_each_kind_total.get(i) - the_rest
                                after_discount_each_kind_total.update({i : number_with_n_digits_after_the_point(round(the_new_amount , 10))})
                                discounting_status = True
                                break
        ...
        for i in each_kind_total:
            the_amount = each_kind_total.get(i)
            the_percentage_of_the_charge_on_totals_under_1 = the_percentage_of_the_charge_on_totals / 100
            the_final_amount = the_amount + (the_amount * the_percentage_of_the_charge_on_totals_under_1) # here is the problem
            after_charge_each_kind_total.update({i : number_with_n_digits_after_the_point(round(the_final_amount , 10))})


        for i in each_tax_total:
            the_amount = each_tax_total.get(i)
            the_percentage_of_the_charge_on_totals_under_1 = the_percentage_of_the_charge_on_totals / 100
            the_final_amount = the_amount + (the_amount * the_percentage_of_the_charge_on_totals_under_1)
            after_charge_each_tax_total.update({i : number_with_n_digits_after_the_point(round(the_final_amount , 10))})
        ...
        if charge_on_totals.get("charge_type") == "amount":
            ...
            the_charged_amount = round(calculate_the_total_after_charge() - calculate_the_total_without_tax() , 10)
            ...
            while the_charge_amount > the_charged_amount:
                ...
                if the_charge_amount > the_charged_amount:
                    ...
                    for i in after_charge_each_tax_total:
                        ...
                        after_charge_each_tax_total.update({i : number_with_n_digits_after_the_point(round(the_new_amount , 10))})
                    ...
                    if not charging_status:
                        for i in after_charge_each_kind_total:
                            ...
                            after_charge_each_kind_total.update({i : number_with_n_digits_after_the_point(round(the_new_amount , 10))})
                            charging_status = True
                            break
        ...
        def create_the_encryption_tags():
            ...
            final_encryption_tags.append("<ext:ExtensionURI>urn:oasis:names:specification:ubl:dsig:enveloped:xades</ext:ExtensionURI>")
            final_encryption_tags.append("<ext:ExtensionContent>")
            final_encryption_tags.append("<sig:UBLDocumentSignatures xmlns:sig=\"urn:oasis:names:specification:ubl:schema:xsd:CommonSignatureComponents-2\" xmlns:sac=\"urn:oasis:names:specification:ubl:schema:xsd:SignatureAggregateComponents-2\" xmlns:sbc=\"urn:oasis:names:specification:ubl:schema:xsd:SignatureBasicComponents-2\">")
            final_encryption_tags.append("<sac:SignatureInformation>")
            final_encryption_tags.append("<cbc:ID>urn:oasis:names:specification:ubl:signature:1</cbc:ID>")
            final_encryption_tags.append("<sbc:ReferencedSignatureID>urn:oasis:names:specification:ubl:signature:Invoice</sbc:ReferencedSignatureID>")
            final_encryption_tags.append("<ds:Signature xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\" Id=\"signature\">")
            final_encryption_tags.append("<ds:SignedInfo>")
            final_encryption_tags.append("<ds:CanonicalizationMethod Algorithm=\"http://www.w3.org/2006/12/xml-c14n11\"/>")
            final_encryption_tags.append("<ds:SignatureMethod Algorithm=\"http://www.w3.org/2001/04/xmldsig-more#ecdsa-sha256\"/>")
            final_encryption_tags.append("<ds:Reference Id=\"invoiceSignedData\" URI=\"\">")
            final_encryption_tags.append("<ds:Transforms>")
            final_encryption_tags.append("<ds:Transform Algorithm=\"http://www.w3.org/TR/1999/REC-xpath-19991116\">")
            final_encryption_tags.append("<ds:XPath>not(//ancestor-or-self::ext:UBLExtensions)</ds:XPath>")
            final_encryption_tags.append("</ds:Transform>")
            final_encryption_tags.append("<ds:Transform Algorithm=\"http://www.w3.org/TR/1999/REC-xpath-19991116\">")
            final_encryption_tags.append("<ds:XPath>not(//ancestor-or-self::cac:Signature)</ds:XPath>")
            final_encryption_tags.append("</ds:Transform>")
            final_encryption_tags.append("<ds:Transform Algorithm=\"http://www.w3.org/TR/1999/REC-xpath-19991116\">")
            final_encryption_tags.append("<ds:XPath>not(//ancestor-or-self::cac:AdditionalDocumentReference[cbc:ID='QR'])</ds:XPath>")
            final_encryption_tags.append("</ds:Transform>")
            final_encryption_tags.append("<ds:Transform Algorithm=\"http://www.w3.org/2006/12/xml-c14n11\"/>")
            final_encryption_tags.append("</ds:Transforms>")
            final_encryption_tags.append("<ds:DigestMethod Algorithm=\"http://www.w3.org/2001/04/xmlenc#sha256\"/>")
            final_encryption_tags.append("<ds:DigestValue></ds:DigestValue>")
            final_encryption_tags.append("</ds:Reference>")
            final_encryption_tags.append("<ds:Reference Type=\"http://www.w3.org/2000/09/xmldsig#SignatureProperties\" URI=\"#xadesSignedProperties\">")
            final_encryption_tags.append("<ds:DigestMethod Algorithm=\"http://www.w3.org/2001/04/xmlenc#sha256\"/>")
            final_encryption_tags.append("<ds:DigestValue></ds:DigestValue>")
            final_encryption_tags.append("</ds:Reference>")
            final_encryption_tags.append("</ds:SignedInfo>")
            ...
            final_encryption_tags.append("<xades:QualifyingProperties xmlns:xades=\"http://uri.etsi.org/01903/v1.3.2#\" Target=\"signature\">")
            final_encryption_tags.append("<xades:SignedProperties Id=\"xadesSignedProperties\">")
        ...
        def create_the_tags_of_the_information():
            ...
            final_information_tags.append("<cbc:ID>{}</cbc:ID>".format(invoice_data.get("invoice_number")))
            final_information_tags.append("<cbc:UUID>{}</cbc:UUID>".format(invoice_data.get("invoice_uuid")))
            final_information_tags.append("<cbc:IssueDate>{}</cbc:IssueDate>".format(invoice_data.get("invoice_issue_date")))
            final_information_tags.append("<cbc:IssueTime>{}</cbc:IssueTime>".format(invoice_data.get("invoice_issue_time")))
            final_information_tags.append("<cbc:InvoiceTypeCode name=\"{}\">{}</cbc:InvoiceTypeCode>".format(invoice_data.get("invoice_type_code"),invoice_data.get("invoice_type_number")))
            final_information_tags.append("<cbc:Note languageID=\"ar\">ABC</cbc:Note>")
            final_information_tags.append("<cbc:DocumentCurrencyCode>SAR</cbc:DocumentCurrencyCode>")
        ...
        def create_the_tag_of_the_source_invoice():
            if invoice_data.get("billingreference_number"):
                ...
                final_source_tags.append("<cbc:ID>{}</cbc:ID>".format(invoice_data.get("billingreference_number")))
                final_source_tags.append("</cac:InvoiceDocumentReference>")
                final_source_tags.append("</cac:BillingReference>")
        ...
        def create_counter_tag():
            ...
            final_counter_tags.append("<cbc:UUID>{}</cbc:UUID>".format(invoice_data.get("the_counter")))
            final_counter_tags.append("</cac:AdditionalDocumentReference>")
        ...
        def create_PIH_tag():
            ...
            final_PIH_tag.append("<cbc:EmbeddedDocumentBinaryObject mimeCode=\"text/plain\">{}</cbc:EmbeddedDocumentBinaryObject>".format(invoice_data.get("the_PIH")))
            final_PIH_tag.append("</cac:Attachment>")
            final_PIH_tag.append("</cac:AdditionalDocumentReference>")
        ...
        def create_QR_tag():
            ...
            final_QR_tag.append("<cbc:EmbeddedDocumentBinaryObject mimeCode=\"text/plain\"></cbc:EmbeddedDocumentBinaryObject>")
            final_QR_tag.append("</cac:Attachment>")
            final_QR_tag.append("</cac:AdditionalDocumentReference>")
        ...
        def create_signature_tag():
            ...
            final_signature_tag.append("<cbc:ID>urn:oasis:names:specification:ubl:signature:Invoice</cbc:ID>")
            final_signature_tag.append("<cbc:SignatureMethod>urn:oasis:names:specification:ubl:dsig:enveloped:xades</cbc:SignatureMethod>")
            final_signature_tag.append("</cac:Signature>")
        ...
        def create_supplier_party_tag():
            ...
            if ((seller_data.get("type_of_PartyIdentification")) and (seller_data.get("PartyIdentification"))) or seller_data.get("StreetName") or seller_data.get("AdditionalStreetName") or seller_data.get("BuildingNumber") or seller_data.get("PlotIdentification") or seller_data.get("CitySubdivisionName") or seller_data.get("CityName") or seller_data.get("PostalZone") or seller_data.get("CountrySubentity") or seller_data.get("country_IdentificationCode") or seller_data.get("vat_registration_number_CompanyID") or seller_data.get("RegistrationName"):
                final_supplier_party_tag.append("<cac:Party>")

                if (seller_data.get("type_of_PartyIdentification")) and (seller_data.get("PartyIdentification")):
                    final_supplier_party_tag.append("<cac:PartyIdentification>")
                    final_supplier_party_tag.append("<cbc:ID schemeID=\"{}\">{}</cbc:ID>".format(seller_data.get("type_of_PartyIdentification"),seller_data.get("PartyIdentification")))
                    final_supplier_party_tag.append("</cac:PartyIdentification>")
                ...
                if seller_data.get("StreetName") or seller_data.get("AdditionalStreetName") or seller_data.get("BuildingNumber") or seller_data.get("PlotIdentification") or seller_data.get("CitySubdivisionName") or seller_data.get("CityName") or seller_data.get("PostalZone") or seller_data.get("CountrySubentity") or seller_data.get("country_IdentificationCode"):
                    ...
                    if seller_data.get("StreetName"):
                        final_supplier_party_tag.append("<cbc:StreetName>{}</cbc:StreetName>".format(seller_data.get("StreetName")))
                    
                    if seller_data.get("AdditionalStreetName"):
                        final_supplier_party_tag.append("<cbc:AdditionalStreetName>{}</cbc:AdditionalStreetName>".format(seller_data.get("AdditionalStreetName")))
                    
                    if seller_data.get("BuildingNumber"):
                        final_supplier_party_tag.append("<cbc:BuildingNumber>{}</cbc:BuildingNumber>".format(seller_data.get("BuildingNumber")))
                    
                    if seller_data.get("PlotIdentification"):
                        final_supplier_party_tag.append("<cbc:PlotIdentification>{}</cbc:PlotIdentification>".format(seller_data.get("PlotIdentification")))
                    
                    if seller_data.get("CitySubdivisionName"):
                        final_supplier_party_tag.append("<cbc:CitySubdivisionName>{}</cbc:CitySubdivisionName>".format(seller_data.get("CitySubdivisionName")))
                    
                    if seller_data.get("CityName"):
                        final_supplier_party_tag.append("<cbc:CityName>{}</cbc:CityName>".format(seller_data.get("CityName")))
                    
                    if seller_data.get("PostalZone"):
                        final_supplier_party_tag.append("<cbc:PostalZone>{}</cbc:PostalZone>".format(seller_data.get("PostalZone")))
                    
                    if seller_data.get("CountrySubentity"):
                        final_supplier_party_tag.append("<cbc:CountrySubentity>{}</cbc:CountrySubentity>".format(seller_data.get("CountrySubentity")))
                    
                    if seller_data.get("country_IdentificationCode"):
                        final_supplier_party_tag.append("<cac:Country>")
                        final_supplier_party_tag.append("<cbc:IdentificationCode>{}</cbc:IdentificationCode>".format(seller_data.get("country_IdentificationCode")))
                        final_supplier_party_tag.append("</cac:Country>")
                ...
                if seller_data.get("vat_registration_number_CompanyID"):
                    ...
                    final_supplier_party_tag.append("<cbc:CompanyID>{}</cbc:CompanyID>".format(seller_data.get("vat_registration_number_CompanyID")))
                ...
                if seller_data.get("RegistrationName"):
                    ...
                    final_supplier_party_tag.append("<cbc:RegistrationName>{}</cbc:RegistrationName>".format(seller_data.get("RegistrationName")))
                    final_supplier_party_tag.append("</cac:PartyLegalEntity>")
        ...
        def create_customer_party_tag():
            ...
            if ((customer_data.get("type_of_PartyIdentification")) and (customer_data.get("PartyIdentification"))) or customer_data.get("StreetName") or customer_data.get("AdditionalStreetName") or customer_data.get("BuildingNumber") or customer_data.get("PlotIdentification") or customer_data.get("CitySubdivisionName") or customer_data.get("CityName") or customer_data.get("PostalZone") or customer_data.get("CountrySubentity") or customer_data.get("country_IdentificationCode") or customer_data.get("vat_registration_number_CompanyID") or customer_data.get("RegistrationName"):
                final_customer_party_tag.append("<cac:Party>")

                if (customer_data.get("type_of_PartyIdentification")) and (customer_data.get("PartyIdentification")):
                    final_customer_party_tag.append("<cac:PartyIdentification>")
                    final_customer_party_tag.append("<cbc:ID schemeID=\"{}\">{}</cbc:ID>".format(customer_data.get("type_of_PartyIdentification"),customer_data.get("PartyIdentification")))
                    final_customer_party_tag.append("</cac:PartyIdentification>")
                ...
                if customer_data.get("StreetName") or customer_data.get("AdditionalStreetName") or customer_data.get("BuildingNumber") or customer_data.get("PlotIdentification") or customer_data.get("CitySubdivisionName") or customer_data.get("CityName") or customer_data.get("PostalZone") or customer_data.get("CountrySubentity") or customer_data.get("country_IdentificationCode"):
                    ...
                    if customer_data.get("StreetName"):
                        final_customer_party_tag.append("<cbc:StreetName>{}</cbc:StreetName>".format(customer_data.get("StreetName")))
                    
                    if customer_data.get("AdditionalStreetName"):
                        final_customer_party_tag.append("<cbc:AdditionalStreetName>{}</cbc:AdditionalStreetName>".format(customer_data.get("AdditionalStreetName")))
                    
                    if customer_data.get("BuildingNumber"):
                        final_customer_party_tag.append("<cbc:BuildingNumber>{}</cbc:BuildingNumber>".format(customer_data.get("BuildingNumber")))
                    
                    if customer_data.get("PlotIdentification"):
                        final_customer_party_tag.append("<cbc:PlotIdentification>{}</cbc:PlotIdentification>".format(customer_data.get("PlotIdentification")))
                    
                    if customer_data.get("CitySubdivisionName"):
                        final_customer_party_tag.append("<cbc:CitySubdivisionName>{}</cbc:CitySubdivisionName>".format(customer_data.get("CitySubdivisionName")))
                    
                    if customer_data.get("CityName"):
                        final_customer_party_tag.append("<cbc:CityName>{}</cbc:CityName>".format(customer_data.get("CityName")))
                    
                    if customer_data.get("PostalZone"):
                        final_customer_party_tag.append("<cbc:PostalZone>{}</cbc:PostalZone>".format(customer_data.get("PostalZone")))
                    
                    if customer_data.get("CountrySubentity"):
                        final_customer_party_tag.append("<cbc:CountrySubentity>{}</cbc:CountrySubentity>".format(customer_data.get("CountrySubentity")))
                    
                    if customer_data.get("country_IdentificationCode"):
                        final_customer_party_tag.append("<cac:Country>")
                        final_customer_party_tag.append("<cbc:IdentificationCode>{}</cbc:IdentificationCode>".format(customer_data.get("country_IdentificationCode")))
                        final_customer_party_tag.append("</cac:Country>")
                ...
                if customer_data.get("vat_registration_number_CompanyID"):
                    ...
                    final_customer_party_tag.append("<cbc:CompanyID>{}</cbc:CompanyID>".format(customer_data.get("vat_registration_number_CompanyID")))
                ...
                if customer_data.get("RegistrationName"):
                    ...
                    final_customer_party_tag.append("<cbc:RegistrationName>{}</cbc:RegistrationName>".format(customer_data.get("RegistrationName")))
                    final_customer_party_tag.append("</cac:PartyLegalEntity>")
        ...
        def create_delivery_tag():
            ...
            final_delivery_means_tag.append("<cbc:ActualDeliveryDate>{}</cbc:ActualDeliveryDate>".format(delivery_data.get("actual_delivery_date")))
        ...
        def create_payment_means_tag():
            ...
            final_payment_means_tag.append("<cbc:PaymentMeansCode>{}</cbc:PaymentMeansCode>".format(payment_data.get("payment_method")))

            if payment_data.get("payment_reason"):
                final_payment_means_tag.append("<cbc:InstructionNote>{}</cbc:InstructionNote>".format(payment_data.get("payment_reason")))
        ...
        def create_allowance_charge_tag():
            ...
            for i in each_kind_total:
                ...
                final_allowance_charge_tag.append("<cbc:ChargeIndicator>true</cbc:ChargeIndicator>")
                final_allowance_charge_tag.append("<cbc:AllowanceChargeReasonCode>CG</cbc:AllowanceChargeReasonCode>")
                final_allowance_charge_tag.append("<cbc:AllowanceChargeReason>Cleaning</cbc:AllowanceChargeReason>")

                if charge_on_totals.get("charge_type") == "percentage":
                    final_allowance_charge_tag.append("<cbc:MultiplierFactorNumeric>{}</cbc:MultiplierFactorNumeric>".format(the_charge_amount))
                    final_allowance_charge_tag.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(the_charged_amount))
                    final_allowance_charge_tag.append("<cbc:BaseAmount currencyID=\"SAR\">{}</cbc:BaseAmount>".format(the_amount_before_charge))
                else:
                    final_allowance_charge_tag.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(the_charged_amount))

                final_allowance_charge_tag.append("<cac:TaxCategory>")
                final_allowance_charge_tag.append("<cbc:ID schemeID=\"UN/ECE 5305\" schemeAgencyID=\"6\">{}</cbc:ID>".format(i))
                final_allowance_charge_tag.append("<cbc:Percent>0.0</cbc:Percent>")
                final_allowance_charge_tag.append("<cac:TaxScheme>")
                final_allowance_charge_tag.append("<cbc:ID schemeID=\"UN/ECE 5153\" schemeAgencyID=\"6\">VAT</cbc:ID>")
                final_allowance_charge_tag.append("</cac:TaxScheme>")
                final_allowance_charge_tag.append("</cac:TaxCategory>")
                final_allowance_charge_tag.append("</cac:AllowanceCharge>")
            ...
            for i in each_tax_total:
                ...
                final_allowance_charge_tag.append("<cac:TaxCategory>")
                final_allowance_charge_tag.append("<cbc:ID schemeID=\"UN/ECE 5305\" schemeAgencyID=\"6\">S</cbc:ID>")
            ...
            for i in each_kind_total:
                ...
                final_allowance_charge_tag.append("<cac:AllowanceCharge>")
                final_allowance_charge_tag.append("<cbc:ChargeIndicator>false</cbc:ChargeIndicator>")
                final_allowance_charge_tag.append("<cbc:AllowanceChargeReasonCode>95</cbc:AllowanceChargeReasonCode>")
                final_allowance_charge_tag.append("<cbc:AllowanceChargeReason>Discount</cbc:AllowanceChargeReason>")

                if discount_on_totals.get("discount_type") == "percentage":
                    final_allowance_charge_tag.append("<cbc:MultiplierFactorNumeric>{}</cbc:MultiplierFactorNumeric>".format(the_discount_amount))
                    final_allowance_charge_tag.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(the_discounted_amount))
                    final_allowance_charge_tag.append("<cbc:BaseAmount currencyID=\"SAR\">{}</cbc:BaseAmount>".format(the_amount_before_discount))
                else:
                    final_allowance_charge_tag.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(the_discounted_amount))
        ...
        def create_tax_total_tags():
            ...
            final_tax_total_tags.append("<cbc:TaxAmount currencyID=\"SAR\">{}</cbc:TaxAmount>".format(calculate_the_tax()))
            final_tax_total_tags.append("</cac:TaxTotal>")
        ...
        def create_legal_monetary_total_tag():
            ...
            the_discounted_value = number_with_n_digits_after_the_point(round(calculate_the_total_without_tax() - calculate_the_total_after_discount() , 10))
            the_charged_value = number_with_n_digits_after_the_point(round(calculate_the_total_after_charge() - calculate_the_total_without_tax() , 10))
            the_total_without_tax = number_with_n_digits_after_the_point(round(calculate_the_total_without_tax() + the_charged_value - the_discounted_value , 10))
            the_total_with_tax = number_with_n_digits_after_the_point(round(calculate_the_tax() + calculate_the_total_without_tax() + the_charged_value - the_discounted_value , 10))

            final_legal_monetary_total_tag.append("<cac:LegalMonetaryTotal>")
            final_legal_monetary_total_tag.append("<cbc:LineExtensionAmount currencyID=\"SAR\">{}</cbc:LineExtensionAmount>".format(calculate_the_total_without_tax()))
            final_legal_monetary_total_tag.append("<cbc:TaxExclusiveAmount currencyID=\"SAR\">{}</cbc:TaxExclusiveAmount>".format(the_total_without_tax))
            final_legal_monetary_total_tag.append("<cbc:TaxInclusiveAmount currencyID=\"SAR\">{}</cbc:TaxInclusiveAmount>".format(the_total_with_tax))
            final_legal_monetary_total_tag.append("<cbc:AllowanceTotalAmount currencyID=\"SAR\">{}</cbc:AllowanceTotalAmount>".format(the_discounted_value))
            final_legal_monetary_total_tag.append("<cbc:ChargeTotalAmount currencyID=\"SAR\">{}</cbc:ChargeTotalAmount>".format(the_charged_value))
            final_legal_monetary_total_tag.append("<cbc:PrepaidAmount currencyID=\"SAR\">0.0</cbc:PrepaidAmount>")
            final_legal_monetary_total_tag.append("<cbc:PayableAmount currencyID=\"SAR\">{}</cbc:PayableAmount>".format(the_total_with_tax))
            final_legal_monetary_total_tag.append("</cac:LegalMonetaryTotal>")
        ...
        def create_a_product_tag():
            ...
            for product in products:
                ...
                product_price = number_with_n_digits_after_the_point(round(product.get("final_price") , 10))
                ...
                product_total_without_tax = number_with_n_digits_after_the_point(round(product_price * product_quantity , 10))
                ...
                product_total_without_tax_with_discount_and_charge = number_with_n_digits_after_the_point(round((product_total_without_tax + charge_on_total) - discount_on_total , 10))
                product_total_tax = number_with_n_digits_after_the_point(round( product_total_without_tax_with_discount_and_charge * (product_tax / 100) , 10))
                product_total_with_tax = number_with_n_digits_after_the_point(round(product_total_without_tax_with_discount_and_charge + product_total_tax , 10))


                the_final_tag_of_the_product_as_list = []
                the_final_tag_of_the_product_as_list.append("<cac:InvoiceLine>")
                the_final_tag_of_the_product_as_list.append("<cbc:ID>{}</cbc:ID>".format(id_number))
                the_final_tag_of_the_product_as_list.append("<cbc:InvoicedQuantity unitCode=\"PCE\">{}</cbc:InvoicedQuantity>".format(product_quantity))
                the_final_tag_of_the_product_as_list.append("<cbc:LineExtensionAmount currencyID=\"SAR\">{}</cbc:LineExtensionAmount>".format(product_total_without_tax_with_discount_and_charge))


                if product.get("charge_on_total"):
                    if product.get("charge_on_total").get("charge_value") > 0:
                        the_final_tag_of_the_product_as_list.append("<cac:AllowanceCharge>")
                        the_final_tag_of_the_product_as_list.append("<cbc:ChargeIndicator>true</cbc:ChargeIndicator>")
                        the_final_tag_of_the_product_as_list.append("<cbc:AllowanceChargeReasonCode>CG</cbc:AllowanceChargeReasonCode>")
                        the_final_tag_of_the_product_as_list.append("<cbc:AllowanceChargeReason>Cleaning</cbc:AllowanceChargeReason>")
                ...
                if product.get("discount_on_total"):
                    if product.get("discount_on_total").get("discount_value") > 0:
                        the_final_tag_of_the_product_as_list.append("<cac:AllowanceCharge>")
                        the_final_tag_of_the_product_as_list.append("<cbc:ChargeIndicator>false</cbc:ChargeIndicator>")
                        the_final_tag_of_the_product_as_list.append("<cbc:AllowanceChargeReasonCode>95</cbc:AllowanceChargeReasonCode>")
                        the_final_tag_of_the_product_as_list.append("<cbc:AllowanceChargeReason>Discount</cbc:AllowanceChargeReason>")

                        if product.get("discount_on_total").get("discount_type") == "percentage":
                            the_final_tag_of_the_product_as_list.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(discount_on_total))
                ...
                the_final_tag_of_the_product_as_list.append("<cbc:TaxAmount currencyID=\"SAR\">{}</cbc:TaxAmount>".format(product_total_tax))
                the_final_tag_of_the_product_as_list.append("<cbc:RoundingAmount currencyID=\"SAR\">{}</cbc:RoundingAmount>".format(product_total_with_tax))
                the_final_tag_of_the_product_as_list.append("</cac:TaxTotal>")

                the_final_tag_of_the_product_as_list.append("<cac:Item>")
                the_final_tag_of_the_product_as_list.append("<cbc:Name>{}</cbc:Name>".format(product_name))

                if product.get("number"):
                    the_final_tag_of_the_product_as_list.append("<cac:SellersItemIdentification>")
                    the_final_tag_of_the_product_as_list.append("<cbc:ID>{}</cbc:ID>".format(product_number))
                    the_final_tag_of_the_product_as_list.append("</cac:SellersItemIdentification>")
                
                the_final_tag_of_the_product_as_list.append("<cac:ClassifiedTaxCategory>")
                the_final_tag_of_the_product_as_list.append("<cbc:ID>{}</cbc:ID>".format(product_tax_id_letter))
                the_final_tag_of_the_product_as_list.append("<cbc:Percent>{}</cbc:Percent>".format(product_tax))
                ...
                the_final_tag_of_the_product_as_list.append("<cbc:PriceAmount currencyID=\"SAR\">{}</cbc:PriceAmount>".format(product_price))
                the_final_tag_of_the_product_as_list.append("<cbc:BaseQuantity unitCode=\"PCE\">1</cbc:BaseQuantity>")
                ...
                if product.get("discount_on_unit"):
                    if product.get("discount_on_unit").get("discount_value") > 0:
                        ...
                        if product.get("discount_on_unit").get("discount_type") == "percentage":

                            if (number_with_n_digits_after_the_point(round( (product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) , 10)) - number_with_n_digits_after_the_point(round( ((product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) * (product.get("discount_on_unit").get("discount_value")/100)) , 10))) == (product.get("final_price")):
                                the_final_tag_of_the_product_as_list.append("<cbc:MultiplierFactorNumeric>{}</cbc:MultiplierFactorNumeric>".format(product.get("discount_on_unit").get("discount_value")))
                                the_final_tag_of_the_product_as_list.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(number_with_n_digits_after_the_point(round( ((product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) * (product.get("discount_on_unit").get("discount_value")/100)) , 10))))
                                the_final_tag_of_the_product_as_list.append("<cbc:BaseAmount currencyID=\"SAR\">{}</cbc:BaseAmount>".format(number_with_n_digits_after_the_point(round( (product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) , 10))))
                            else:
                                the_final_tag_of_the_product_as_list.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(number_with_n_digits_after_the_point(round( (number_with_n_digits_after_the_point(round( (product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) , 10)) - product.get("final_price") ) , 10))))
                                the_final_tag_of_the_product_as_list.append("<cbc:BaseAmount currencyID=\"SAR\">{}</cbc:BaseAmount>".format(number_with_n_digits_after_the_point(round( (product.get("final_price") / (1 - (product.get("discount_on_unit").get("discount_value")/100))) , 10))))
                        else:
                            the_final_tag_of_the_product_as_list.append("<cbc:Amount currencyID=\"SAR\">{}</cbc:Amount>".format(product.get("discount_on_unit").get("discount_value")))
                            the_final_tag_of_the_product_as_list.append("<cbc:BaseAmount currencyID=\"SAR\">{}</cbc:BaseAmount>".format(number_with_n_digits_after_the_point(round(product.get("final_price") + product.get("discount_on_unit").get("discount_value") , 10))))
                ...
                the_final_tag_of_the_product_as_string = "\n".join(the_final_tag_of_the_product_as_list)
                the_final_tags_of_the_products_as_list.append(the_final_tag_of_the_product_as_string)
        ...
        the_final_invoice_as_list.append("<Invoice xmlns=\"urn:oasis:names:specification:ubl:schema:xsd:Invoice-2\" xmlns:cac=\"urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2\" xmlns:cbc=\"urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2\" xmlns:ext=\"urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2\">")
...
def zid_json_to_invoice(request):
    ...
    customer_building_number = json_data['shipping']['address']['meta']['building_number']
    customer_additional_number = json_data['shipping']['address']['meta']['additional_number']
...
def identify_webhook_type(webhook_data):
    if 'app_id' in webhook_data and 'status' in webhook_data and 'event_name' in webhook_data:
        return 'app_installation'
    elif 'id' in webhook_data and 'order_url' in webhook_data and 'customer' in webhook_data:
        return 'order'
    else:
        return 'unknown'
    ...
    if request.method == 'POST':
        try:
            ...
            if webhook_type == 'app_installation':
                ...
                date_part, time_part = issue_date.split(" | ")

                # Parse the date part into a datetime object
                date_object = datetime.datetime.strptime(date_part, "%d-%m-%Y")
                print("Date:", date_object.date())


                time_string = time_part[:-1]

                # Handle potential leading zeros and seconds
                time_string = time_string.zfill(5)  # Add leading zeros if needed

                try:
                    time_object = datetime.datetime.strptime(time_string, "%H:%M").time()
                except ValueError:
                    # Handle potential errors, like incorrect format or missing seconds
                    print("Error parsing time:", time_string)
                    time_object = None  # Or handle the error differently
                ...
                invoice_data = {"invoice_number": new_irn,
                                "billingreference_number": "",
                                "invoice_uuid": unique_id,
                                "invoice_issue_date": date_object.date(),
                                "invoice_issue_time":  time_object,
                ...
                seller_data = {"type_of_PartyIdentification":"CRN",
                            "PartyIdentification":'',
                            "StreetName":'',
                ...
                customer_data = {"type_of_PartyIdentification": "",
                                "PartyIdentification": '',
                                "StreetName": '',
                                "AdditionalStreetName": '',
                ...
                discount_data = {"discount_type": "amount", # you can put either (amount) or (percentage)
                                "discount_value": total_discount_amount}

                # this charge is on the total of the invoice
                charge_data = {"charge_type": "amount", # you can put either (amount) or (percentage)
                            # "charge_value": product_discounts
                            "charge_value": total_discount_amount
                            }
                delivery_data = {
...
def create_invoice_view(request):
   ...
   "edits_count":0
    ...
    # if request.method == 'POST':
        ...
        vat_item = next(item for item in json_data['payment']['invoice'] if item['code'] == 'vat')
        ...
        # payment_data = {"payment_method": payment_method,
        #                 "payment_reason": ""}
        payment_data = {"payment_method": "10",
